...
#define RED_LED		 	11			// Red LED to digital port (DP) 11
#define GREEN_LED	 	8			// Green LED to DP 8
#define BLUE_LED	 	5			// Blue LED to DP 4
#define LIGHT_PIN		A0			// Light sensor to analog port A0
#define TEMP_PIN 		A1			// Temp sensor to analog port A1

#define WAIT_INTERVAL 1000			// Wait time 1 second 
...
void loop() {
  ...
  /* Simple percentage conversion for light (0-100%)
  The analog reading (0-1023) is normally divided by 10.23 to get percentage
  Like so: light_value = analogRead(LIGHT) / 10.23;
  
  However, for TinkerCad, after empirical testing, for phototransistor the max
  value is 471 and for photoresistor the max value is 679, with current resistor,
  hence dividing or using map with 471 and 679 respectively to map max value to 100. */
  light_value = map(analogRead(LIGHT_PIN),0, 679, 0, 100);
  
  // ---- Temperature Sensor Conversion ----
  /* For TMP36 sensor, using a more straightforward conversion:
  1. Read analog value (0-1023)
  2. Convert to voltage (0-5V): analogValue * (5.0 / 1023.0)
  3. Convert voltage to Celsius: (voltage - 0.5) * 100 */
